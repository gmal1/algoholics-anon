function shortestPath(matrix) {
  ...
  while(queue.length) {
    ...
    const up = row - 1 >= 0 && !visited[row - 1][col] ? matrix[row - 1][col] : 0;
    const down = row + 1 < matrix.length && !visited[row + 1][col] ? matrix[row + 1][col] : 0;
    const left = col - 1 >= 0 && !visited[row][col - 1] ? matrix[row][col - 1] : 0;
    const right = col + 1 < matrix[row].length && !visited[row][col + 1] ? matrix[row][col + 1] : 0;
